...
class SubjectReader
{
    ...
    {
        if (!isset($subject[Config::GENOME_ORDER_STATUS])) {
            throw new InvalidArgumentException('The "status" field does not exists');
        }

        return $subject[Config::GENOME_ORDER_STATUS];
    }
    ...
    {
        return $subject[Config::GENOME_BILL_TOKEN] ?? null;
    }
    ...
    {
        if (!isset($subject[Config::GENOME_TRANSACTION_ID])) {
            throw new InvalidArgumentException('The "transactionId" field does not exists');
        }

        return $subject[Config::GENOME_TRANSACTION_ID];
    }
    ...
    {
        if (!isset($subject[Config::GENOME_ORDER_AMOUNT])) {
            throw new InvalidArgumentException('The "amount" field does not exists');
        }

        return $subject[Config::GENOME_ORDER_AMOUNT];
    }
    ...
    {
        if (!isset($subject[Config::GENOME_ORDER_CURRENCY])) {
            throw new InvalidArgumentException('The "currency" field does not exists');
        }

        return $subject[Config::GENOME_ORDER_CURRENCY];
    }
    ...
    {
        if (!isset($subject[Config::GENOME_RESPONSE_MESSAGE])) {
            throw new InvalidArgumentException('The "message" field does not exists');
        }

        return $subject[Config::GENOME_RESPONSE_MESSAGE];
    }
}
