...
namespace Genome\Payment\Controller\Iframe;

use Exception;
use Genome\Payment\Gateway\Config\Config;
use Genome\Payment\Model\CallbackDataMapper;
use LogicException;
...
class Callback implements HttpPostActionInterface, CsrfAwareActionInterface
{
    ...
    public function execute(): ResultInterface
    {
        try {
            ...
            if ($orderPayment->getMethod() !== Config::CODE) {
                throw new LogicException(
                    'Order Payment Method does not supported by Genome Payment.'
                );
            }
        } catch (Exception $e) {
        ...
        if (!in_array($orderStatus, [Config::GENOME_ORDER_STATUS_REFUND, Config::GENOME_ORDER_STATUS_DECLINE], true)) {
